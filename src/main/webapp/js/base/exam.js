...
//			}},
			...
			{ label: '失败原因', name: 'error',index:'error', width: 45 }
        ],
		viewrecords: true,
        height: 400,
        rowNum: 10,
...
function reloadGrid(){
		$("#jqGrid").jqGrid('setGridParam', {
			...
			postData : {
				...
				empName : $("#empName").val(),
				idCode : $("#idCode").val(),
				kjdBackTimeout : $("#kjdBackTimeout").val(),
				reportDataTimeout : $("#reportDataTimeout").val()
			}
		}).trigger("reloadGrid");
}
var vm = new Vue({
	...
	methods: {
		...
		notice: function (event) {
			...
			var ret=$("#jqGrid").jqGrid('getRowData', examId);
			//alert(ret.examNo);
			$.ajax({
				type: "POST",
				contentType :  "application/json;charset=utf-8",
			    url: "/"+rootPath+"/noticeUploadReport.do",
