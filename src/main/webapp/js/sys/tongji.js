var exapOrgParentName = T.p("exapOrgParentName");
$(function () {
    $("#jqGrid").jqGrid({
        url: '/'+rootPath+'/tongji.do',
        datatype: "json",
        postData : {"exapOrgParentName" : exapOrgParentName},
        colModel: [			
			...
			{ label: '已预约', name: 'appCount',index:'appCount', width: 75 ,formatter:function(value, options, row){
				...
				if(row.exapOrgName=='合计'){
					var url="/"+rootPath+"/sys/exam.html?exapOrgName="+""+"&examStatus=1";
			...
			{ label: '取消预约', name: 'cancelCount',index:'cancelCount', width: 75 ,formatter:function(value, options, row){
				...
				if(row.exapOrgName=='合计'){
					var url="/"+rootPath+"/sys/exam.html?exapOrgName="+""+"&examStatus=2";
					return "<a href='"+url+"'>"+value+"</a>";
				}
				var url="/"+rootPath+"/sys/exam.html?exapOrgName="+escape(row.exapOrgName)+"&examStatus=2";
				return "<a href='"+url+"'>"+value+"</a>";
			}},		
			{ label: '体检报告已回传', name: 'reportCount',index:'reportCount', width: 75,formatter:function(value, options, row){
				...
				if(row.exapOrgName=='合计'){
					var url="/"+rootPath+"/sys/exam.html?exapOrgName="+""+"&examStatus=3";
					return "<a href='"+url+"'>"+value+"</a>";
				}
				var url="/"+rootPath+"/sys/exam.html?exapOrgName="+escape(row.exapOrgName)+"&examStatus=3";
				return "<a href='"+url+"'>"+value+"</a>";
			}},		
			{ label: '体检报告已上传至KJD', name: 'uoloadCount',index:'uoloadCount', width: 75 ,formatter:function(value, options, row){
				...
				if(row.exapOrgName=='合计'){
					var url="/"+rootPath+"/sys/exam.html?exapOrgName="+""+"&examStatus=4";
					return "<a href='"+url+"'>"+value+"</a>";
				}
				var url="/"+rootPath+"/sys/exam.html?exapOrgName="+escape(row.exapOrgName)+"&examStatus=4";
				return "<a href='"+url+"'>"+value+"</a>";
			}},
			{ label: '体检分析数据已取回', name: 'fetchDataCount',index:'fetchDataCount', width: 75  ,formatter:function(value, options, row){
				...
				if(row.exapOrgName=='合计'){
					var url="/"+rootPath+"/sys/exam.html?exapOrgName="+""+"&examStatus=5";
					return "<a href='"+url+"'>"+value+"</a>";
				}
				var url="/"+rootPath+"/sys/exam.html?exapOrgName="+escape(row.exapOrgName)+"&examStatus=5";
				return "<a href='"+url+"'>"+value+"</a>";
			}},		
			{ label: '总数量', name: 'totalCount',index:'totalCount', width: 90 ,formatter:function(value, options, row){
				...
				if(row.exapOrgName=='合计'){
					var url="/"+rootPath+"/sys/exam.html";
					return "<a href='"+url+"'>"+value+"</a>";
				}
				var url="/"+rootPath+"/sys/exam.html?exapOrgName="+escape(row.exapOrgName)+"";
				return "<a href='"+url+"'>"+value+"</a>";
			}}
        ],
		viewrecords: true,
...
var vm = new Vue({
	...
	methods: {
		...
		getPhdictitems:function(event){
            $.get('/'+rootPath+"/phDictItem.do?dictType=phexamorg", function(r){
            	vm.phdictitems = r;
    		});
		},
		getPhdictitemsParent:function(event){
            $.get('/'+rootPath+"/phDictItem.do?dictType=phexamorgtype", function(r){
            	vm.phdictitemsParent = r;
    		});
		},
